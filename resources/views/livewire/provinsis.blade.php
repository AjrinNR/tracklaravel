<div>
            ...
            <div class="col-md-12">
                <select wire:model="selectedKota" class="form-control" name="id_kota">
                    ...
                    @foreach($kota as $data2)
                        <option value="{{ $data2->id_prov }}">{{ $data2->nama_kota }}</option>
                    @endforeach
                </select>
            </div>
        </div>
            ...
            <div class="col-md-12">
                <select wire:model="selectedKecamatan" class="form-control" name="id_kec">
                    ...
                    @foreach($kecamatan as $data3)
                        <option value="{{ $data3->id_kota }}">{{ $data3->nama_kec }}</option>
                    @endforeach
                </select>
            </div>
        </div>
    ...
    <div class="form-group row">
            ...
            <div class="col-md-12">
                <select wire:model="selectedKelurahan" class="form-control" name="id_kec">
                    <option value="" selected>Nama Kelurahan</option>
                    @foreach($kelurahan as $data4)
                        <option value="{{ $data4->id_kec}}">{{ $data4->nama_kelurahan }}</option>
                    @endforeach
                </select>
            </div>
        </div>
